//===- llvm/unittest/ADT/IntrusiveVariantTest.cpp - IntrusiveVariant test -===//
//
...
TEST(IntrusiveVariantTest, ConstructionAndAssignment) {
  IntrusiveVariant<I, F, D> V{std::in_place_type<F>, 2.0f};
...
TEST(IntrusiveVariantTest, Comparison) {
  IntrusiveVariant<I, F, D> V{std::in_place_type<I>, 1};
  IntrusiveVariant<I, F, D> W{std::in_place_type<F>, 2.0f};
  IntrusiveVariant<I, F, D> X{std::in_place_type<F>, 2.0f};
  IntrusiveVariant<I, F, D> Y{std::in_place_type<F>, 3.0f};
  IntrusiveVariant<I, F, D> Z{std::in_place_type<D>, 3.0};
  EXPECT_NE(V, W);
  EXPECT_LT(V, W);
  EXPECT_LE(V, W);
  EXPECT_GT(W, V);
...
TEST(IntrusiveVariantTest, HoldsAlternative) {
  IntrusiveVariant<I, F, D> V{std::in_place_type<D>, 2.0};
  ...
  const IntrusiveVariant<I, F, D> C{std::in_place_type<F>, 2.0f};
...
TEST(IntrusiveVariantTest, Get) {
  ...
  EXPECT_EQ(&V.get<I>(), V.getIf<I>());
  const IntrusiveVariant<I, F, D> C{std::in_place_type<D>, 2.0};
...
TEST(IntrusiveVariantTest, HashValue) {
  IntrusiveVariant<IntA, IntB> ATwo{std::in_place_type<IntA>, 2};
  IntrusiveVariant<IntA, IntB> AThree{std::in_place_type<IntA>, 3};
  IntrusiveVariant<IntA, IntB> BTwo{std::in_place_type<IntB>, 2};
...
TEST(IntrusiveVariantTest, HeaderExample) {
  ...
  MyVariant Variant{std::in_place_type<AltComplexInt>, 4, 2};
