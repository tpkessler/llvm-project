...
//
// We use std::in_place_index_t here both to disambiguate the constructor and to
// make defining the overload set for getMember more natural.
template <size_t Index, typename HeadT, typename... TailTs> union UnionImpl {
  using TailT = UnionImpl<Index + 1, TailTs...>;
  HeadT Head;
  TailT Tail;
  HeadT &getMember(std::in_place_index_t<Index>) { return Head; }
  const HeadT &getMember(std::in_place_index_t<Index>) const { return Head; }
  template <size_t I> decltype(auto) getMember(std::in_place_index_t<I>) {
    return Tail.getMember(std::in_place_index<I>);
  }
  template <size_t I> decltype(auto) getMember(std::in_place_index_t<I>) const {
    return Tail.getMember(std::in_place_index<I>);
  }
  template <typename... ArgTs>
  UnionImpl(std::in_place_index_t<Index>, ArgTs &&...Args) {
    new (&Head) HeadT(std::forward<ArgTs>(Args)...);
    Head.IntrusiveVariantTagMember.Index = Index;
  }
  template <size_t I, typename... ArgTs>
  UnionImpl(std::in_place_index_t<I>, ArgTs &&...Args) {
    new (&Tail) TailT(std::in_place_index_t<I>{}, std::forward<ArgTs>(Args)...);
  }
...
private:
  static_assert(std::conjunction<std::is_standard_layout<Ts>...>::value,
  ...
  // Helper to get the std::in_place_index_t for T in Ts...
  template <typename T>
  using InPlaceIndexT =
      std::in_place_index_t<FirstIndexOfType<T, Ts...>::value>;
...
public:
  ...
  constexpr IntrusiveVariant() : Union(std::in_place_index_t<0>{}) {}
  ...
  template <typename T, std::enable_if_t<IsAlternativeType<T>::value, int> = 0,
            typename... ArgTs>
  explicit constexpr IntrusiveVariant(std::in_place_type_t<T>, ArgTs &&...Args)
