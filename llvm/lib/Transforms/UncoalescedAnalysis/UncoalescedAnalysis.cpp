...
size_t UncoalescedAnalysis::getTypeSize(const Type *ty, const DataLayout &DL) const {
  // Note (jobailey): This used to be memoized (likely because you
  // might have needed to drill down trough nested pointer / array
  // types previously). However, this doesn't *seem* necessary w/
  // opaque pointer types, since we're directly grabbing the type
...
//    }
    ...
    // Detect uncoalesced accesses.
    size_t size = getTypeSize(SI->getValueOperand()->getType(),
